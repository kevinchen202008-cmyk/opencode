...
OpenCode 在进程内是单体应用（单 Server 进程），但从职责上可拆成多块功能模块；对外则通过 Monorepo 多包呈现“微服务式”边界（可独立构建、版本、部署的部分）。

**（1）核心包 `packages/opencode` 内部功能模块图**

下图按「领域 + 入口」划分，展示 `packages/opencode/src` 下的主要功能模块及其依赖关系（箭头表示调用或数据流方向）。
...
flowchart TB
  subgraph CLI[CLI 入口层]
    ...
    CmdSession[session]
    CmdMcp[mcp]
    CmdAuth[auth / models / db]
    CmdOther["acp / attach / export / import / pr / github / ..."]
  end

  subgraph Server[Server 与路由]
    Http[Hono HTTP Server]
    SSE["SSE / Event 流"]
    WS[WebSocket]
    Routes["路由: project / session / file / config\nprovider / mcp / pty / tui / permission\nquestion / global / experimental"]
  end
  ...
  subgraph Backing[支撑与集成]
    Provider[Provider 多模型]
    MCP[MCP 客户端]
    Plugin[Plugin 插件]
    Project[Project / Worktree / Instance]
    Storage["Storage\n(SQLite + Drizzle)"]
    Bus[Bus 事件总线]
  end
...
**（2）Monorepo 包级地图（微服务/可独立部署边界）**

以下按「发布单元」划分，展示各包职责及对核心的依赖关系；核心运行时仍以 `packages/opencode` 为主，其余为客户端或 SDK。

```mermaid
flowchart LR
  subgraph Core[核心运行时]
    PkgOpenCode["packages/opencode\n(CLI + Server + Agent + Tool\n+ Provider + MCP + Plugin)"]
  end

  subgraph Clients[客户端与入口]
    PkgApp["packages/app\n(Web UI / SolidJS)"]
    PkgDesktop["packages/desktop\n(Tauri 桌面)"]
    PkgWeb["packages/web\n(文档站 / Astro)"]
    SdkVscode["sdks/vscode\n(VSCode / Cursor 扩展)"]
  end

  subgraph SDKs[SDK 与扩展]
    PkgPlugin["packages/plugin\n(@opencode-ai/plugin)"]
    PkgSdk["packages/sdk/js\n(@opencode-ai/sdk)"]
  end

  subgraph Other[其他功能包]
    PkgUi["packages/ui"]
    PkgUtil["packages/util"]
    PkgScript["packages/script"]
    PkgSlack["packages/slack"]
    PkgConsole["packages/console/*"]
    PkgEnterprise["packages/enterprise"]
  end

  PkgApp --> PkgOpenCode
  PkgDesktop --> PkgApp
  PkgDesktop --> PkgOpenCode
  SdkVscode --> PkgOpenCode
  PkgPlugin --> PkgOpenCode
  PkgSdk --> PkgOpenCode
  PkgUi --> PkgApp
  PkgOpenCode --> PkgUtil
  PkgOpenCode --> PkgScript
  PkgSlack --> PkgOpenCode
  PkgConsole --> PkgOpenCode
  PkgEnterprise --> PkgOpenCode
```

说明：

- **功能模块图**：对应「单进程内」的职责拆分，便于理解 Server、Agent、Tool、Provider、MCP、Plugin、Session、Storage 等如何协同。
- **包级地图**：对应「可独立构建/发布的单元」；实际部署时通常仍将 `packages/opencode` 作为唯一服务进程，其他包作为前端或 CLI/SDK 依赖该进程。
...
OpenCode 将 Git 作为当前唯一支持的 VCS 深度集成：在项目解析、工作区管理、分支感知和沙箱（worktree）创建中调用本地 `git` 命令，并通过统一抽象（`vcs` + `worktree`）和 HTTP API 暴露给前端/TUI。

**（1）项目解析与 VCS 检测**

- **入口**：`packages/opencode/src/project/project.ts` 的 `Project.fromDirectory(directory)`。
- **流程**：
  1. 自 `directory` 向上查找 `.git` 目录（`Filesystem.up({ targets: [".git"], start: directory })`）。
  2. 若找到 `.git` 且本机有 `git` 可执行文件，则：
     - 用 `git rev-list --max-parents=0 --all` 取根提交，生成并缓存项目 ID（存于 `.git/opencode`）。
     - 用 `git rev-parse --show-toplevel` 得到仓库根目录。
     - 用 `git rev-parse --git-common-dir` 区分主工作区与 worktree，得到 `worktree` 路径。
  3. 最终写入内存/DB 的 `Project.Info` 包含 `worktree`、`vcs: "git"`；若无 Git 或命令失败，则 `vcs` 为假值，仍可当“无 VCS”使用。

**（2）统一 Git 调用层**

- **位置**：`packages/opencode/src/util/git.ts`。
- **作用**：封装所有对 `git` 的调用为 `git(args, { cwd, env })`，返回 `{ exitCode, text(), stdout, stderr }`。
- **特殊逻辑**：当 `Flag.OPENCODE_CLIENT === "acp"` 时，使用 `Bun.spawn(["git", ...args], { stdin: "ignore", ... })` 而非 Bun 的 shell，避免 Windows 下 ACP 的 stdin 管道导致 git 死锁。

**（3）VCS 抽象与分支感知**

- **位置**：`packages/opencode/src/project/vcs.ts`。
- **能力**：
  - `Vcs.branch()`：当前分支名（`git rev-parse --abbrev-ref HEAD`，在 `Instance.worktree` 下执行）。
  - 当 `Instance.project.vcs === "git"` 时，通过 FileWatcher 订阅 `.git` 目录下除 `HEAD` 外的变更；若检测到分支变化，发布 `Vcs.Event.BranchUpdated`，TUI/前端可据此刷新显示。
- **HTTP API**：`GET /vcs`，返回 `{ branch }`。TUI 在同步上下文时调用 `sdk.client.vcs.get()`。

**（4）Worktree 沙箱与实验性 API**

- **位置**：`packages/opencode/src/worktree/index.ts`。
- **能力**：
  - **创建**：`Worktree.create(input)` → 执行 `git worktree add --no-checkout -b <branch> <directory>`，并可执行项目配置的 `commands.start`。
  - **列出**：`git worktree list --porcelain` 解析出现有 worktree 列表。
  - **删除**：`git worktree remove --force <path>`，并视情况删除分支。
  - **重置**：将某 worktree 对应分支重置到主默认分支。
- **API**：实验性路由 `server/routes/experimental.ts` 暴露 `POST/GET/DELETE /worktree`、`POST /worktree/reset`。

**（5）文件监听与忽略**

- **FileWatcher**（`packages/opencode/src/file/watcher.ts`）：在 Git 仓库中通过 `git rev-parse --git-dir` 得到 `.git` 路径，并订阅该目录（除 `HEAD` 外）的文件变更，用于驱动 VCS 分支变更事件。
- **FileIgnore**（`packages/opencode/src/file/ignore.ts`）：默认忽略列表中包含 `.git`、`.svn`、`.hg`，避免 Agent/工具把 VCS 元数据当普通文件处理。

**（6）数据流简图**

```mermaid
flowchart LR
  subgraph Detection[项目解析]
    DotGit[".git 检测"]
    GitRevParse["git rev-parse\nrev-list"]
    ProjectDB["Project.Info\nworktree, vcs"]
    DotGit --> GitRevParse
    GitRevParse --> ProjectDB
  end

  subgraph Runtime[运行时]
    Instance[Instance.worktree]
    Vcs[Vcs.branch / Event]
    Worktree["Worktree\ncreate list remove reset"]
